...
// The Theme RGB window: the settings, what lights, and the devices found.
// All state lives in Service.qml (the shell injects it); this file only
// shows it and forwards the user's choices. The bar widget in Panel.qml
// summons this through the shell. The preview strips are painted from the
// same `stops` the script sends to the hardware, so what you see is what
// lights.
Item {
  id: themeRgb
  ...
  readonly property string pluginId: manifest && manifest.id ? String(manifest.id) : "huacnlee.theme_rgb"
  property bool opened: false
  property bool closingFromHost: false

  // Host-driven open (`omarchy-shell shell toggle huacnlee.theme_rgb`, the
  // bar icon). The payload is unused.
  function open(payloadJson) {
    closingFromHost = false
    opened = true
    window.visible = true
    if (service) { service.windowOpen = true; service.refreshVariables(); service.refreshStops() }
    cursorActive = false
    cursorRow = "mode"
  ...
  // User-driven close (Esc, the window's close button): route through the
  // shell so its record of open panels stays right and toggle works next
  // time.
  ...
  function runMenuAction(id) {
    if (id === "sync") syncNow()
    else if (id === "github") Quickshell.execDetached(["xdg-open", githubUrl])
  }
  ...
  FloatingWindow {
    ...
    PanelKeyCatcher {
      ...
      // before it closes the window and j/k walk the list, not the swatches.
      ...
      onCloseRequested: themeRgb.requestClose()
      ...
      // Header across the top; settings on the left, devices on the right.
      Column {
        id: page
        anchors.fill: parent
        ...
        Item {
          ...
          // Welcome, in place of everything, until OpenRGB is installed.
          Column {
            id: welcome
          ...
          Row {
            ...
            // Settings scroll; the device list on the right stays put.
            Flickable {
              id: settingsFlick
